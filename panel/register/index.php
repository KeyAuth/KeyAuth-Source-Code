<?php	define(DEBUG,true);		if(DEBUG)	{		ini_set('display_errors', 1);ini_set('display_startup_errors', 1);error_reporting(E_ALL);	}
...
<html lang="en">
<head>
	<title>KeyAuth - Panel Register</title>
	<meta charset="UTF-8">
  ...
  <?php
        ...
        if (isset($_POST['register']))
        {
            ...
            // para cuando supere el captcha
                ...
                $email = xss_clean(mysqli_real_escape_string($link, $_POST['key']));
                ...
                else
                {
                    ...
                    $email_check = mysqli_query($link, "SELECT `email` FROM `accounts` WHERE `email` = '$email'") or die(mysqli_error($link));
                    ...
                    $key_check = mysqli_query($link, "SELECT * FROM `keys` WHERE `key` = '$email'") or die(mysqli_error($link));
                    $do_key_check = mysqli_num_rows($key_check);

                    if ($do_key_check == 0)
                    {
                        $register_status = "key_token";
                                            echo '
                    <script type=\'text/javascript\'>
                ...
                $row = mysqli_fetch_array($key_check);
                ...
                if($status == "Not Used")
                {
                    ...
                    echo '
                            ...
                            notyf
                              .error({
                                message: \'Your key has not been activated!\',
                                duration: 3500,
                                dismissible: true
                              });                
                ...
                if ($expires < $today)
                {
                    ...
                    if ($register_status == "success")
                    {
                        ...
                        mysqli_query($link, "INSERT INTO `accounts` (`username`, `email`, `password`, `role`, `app`, `owner`, `isbanned`, `img`, `pp`, `dayrate`, `weekrate`, `monthrate`,`balance`) VALUES ('$username', '$email', '$pass_encrypted','panel','','',0,'https://i.imgur.com/TrwYFBa.png', '','','','','1')") or die(mysqli_error($link));
                        $register_status = "success";
                    }
