...
switch ($type)
{
    ...
    case 'addvar':
		if(!is_numeric($_GET['authed']))
			error("Authed paramater must be 1 if you want to require login first, or 0 if you don't want to.");
	...
	case 'muteuser':
		if(!is_numeric($_GET['time']))
			error("Invalid time paramater, must be number");
    ...
    switch ($resp)
    {
        case 'username_taken' : die(json_encode(array(
            "success" => false,
...
case 'extend':
	if(!is_numeric($_GET['expiry']))
	error("Expiry not set correctly, must be number of days");
...
case 'resethash':
	...
	if (mysqli_affected_rows($link) > 0)
		success("Reset hash successfully!");
