...
if (isset($_POST['draw'])) {
	...
	while ($row = mysqli_fetch_assoc($empRecords)) {
		...
		$banBtns = "";
		if ($row['banned'] ? true : false) { $banned = $row['banned']; $banBtns = '<button class="btn menu-link px-3" style="font-size:0.95rem;" name="unbanuser" value="' . $row['username'] . '">Unban</button>'; } else { $banned = "N/A"; $banBtns = '<a class="menu-link px-3" data-bs-toggle="modal" data-bs-target="#ban-user" onclick="banuser(\''.$row["username"].'\')">Ban</a>';}
		
		## Fetch Paused Status.
		$PausedStatus = "";
		$PResult = mysqli_query($link, "SELECT * FROM `subs` WHERE `app` = '" . $_SESSION['app'] . "' AND `user` = '" . $row['username'] . "'");
		$PResultRow = mysqli_fetch_array($PResult);

		if ($PResultRow['paused']) { $PausedStatus = '<button class="btn menu-link px-3" style="font-size:0.95rem;" name="unpauseuser" value="' . $row['username'] . '">Unpause</button>'; } else { $PausedStatus = '<button class="btn menu-link px-3" style="font-size:0.95rem;" name="pauseuser" value="' . $row['username'] . '">Pause</button>';}
