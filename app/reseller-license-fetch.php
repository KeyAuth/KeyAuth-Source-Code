...
if(isset($_POST['draw']) ) {
	...
	$sel = mysqli_query($link,"select count(1) as allcount from `keys` where app = '".$_SESSION['app']."' and genby = '".$_SESSION['username']."'");
	$records = mysqli_fetch_assoc($sel);
	$totalRecords = $records['allcount'];
	
	## Total number of record with filtering
	$sel = mysqli_query($link,"select count(1) as allcount from `keys` WHERE 1 ".$searchQuery." and app = '".$_SESSION['app']."' and genby = '".$_SESSION['username']."'");
	$records = mysqli_fetch_assoc($sel);
	$totalRecordwithFilter = $records['allcount'];
	
	## Fetch records
	$empQuery = "select * from `keys` WHERE 1 ".$searchQuery." and app = '".$_SESSION['app']."' and genby = '".$_SESSION['username']."' order by `".$columnName."` ".$columnSortOrder." limit ".$row.",".$rowperpage;
	// echo $empQuery;
