...
if(isset($_POST['draw']) ) {
	...
	if(!in_array($columnName, array("key", "gendate", "expires", "note", "usedon", "usedby", "status", "actions"))) {
	...
	## Fetch records
	$query = misc\mysql\query("select * from `keys` WHERE 1 and (`key` like ? or `note` like ? or `usedby` like ? ) and app = ? and genby = ? order by `".$columnName."` ".$columnSortOrder." limit " . $row . "," . $rowperpage,["%" . $searchValue . "%", "%" . $searchValue . "%", "%" . $searchValue . "%", $_SESSION['app'], $_SESSION['username']]);
	$data = array();
