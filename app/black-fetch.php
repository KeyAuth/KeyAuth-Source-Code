...
if(isset($_POST['draw']) ) {
	...
	if(!in_array($columnName, array("data", "type", "actions"))) {
	...
	## Fetch records
	$query = misc\mysql\query("select * from `bans` WHERE 1  and (`ip` like ? or `hwid` like ? or `type` like ? ) and app = ? order by `".$columnName."` ".$columnSortOrder." limit " . $row . "," . $rowperpage,["%" . $searchValue . "%", "%" . $searchValue . "%", "%" . $searchValue . "%", $_SESSION['app']]);
	$data = array();
