...
if(isset($_POST['draw']) ) {
	...
	$sel = mysqli_query($link,"select count(1) as allcount from `bans` where app = '".$_SESSION['app']."'");
	$records = mysqli_fetch_assoc($sel);
	$totalRecords = $records['allcount'];
	
	## Total number of record with filtering
	$sel = mysqli_query($link,"select count(1) as allcount from `bans` WHERE 1 ".$searchQuery." and app = '".$_SESSION['app']."'");
	$records = mysqli_fetch_assoc($sel);
	$totalRecordwithFilter = $records['allcount'];
	
	## Fetch records
	$empQuery = "select * from `bans` WHERE 1 ".$searchQuery." and app = '".$_SESSION['app']."' limit ".$row.",".$rowperpage;
	// echo $empQuery;
