...
if(isset($_POST['draw']) ) {
	...
	$sel = misc\mysql\query("select count(1) as allcount from `vars` where app = ?",[$_SESSION['app']]);
	$records = mysqli_fetch_assoc($sel->result);
	$totalRecords = $records['allcount'];
	
	## Total number of record with filtering
	$sel = misc\mysql\query("select count(1) as allcount from `vars` WHERE 1  and (`varid` like ? or `msg` like ? ) and app = ?",["%" . $searchValue . "%", "%" . $searchValue . "%", $_SESSION['app']]);
	$records = mysqli_fetch_assoc($sel->result);
	$totalRecordwithFilter = $records['allcount'];
	
	// whitelist certain column names and sort orders to prevent SQL injection
	if(!in_array($columnName, array("varid", "data", "authed", "actions"))) {
	...
	## Fetch records
	$query = misc\mysql\query("select * from `vars` WHERE 1  and (`varid` like ? or `msg` like ? ) and app = ? order by `".$columnName."` ".$columnSortOrder." limit " . $row . "," . $rowperpage,["%" . $searchValue . "%", "%" . $searchValue . "%", $_SESSION['app']]);
	$data = array();
