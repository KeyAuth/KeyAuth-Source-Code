...
if(isset($_POST['draw']) ) {
	...
	$sel = mysqli_query($link,"select count(1) as allcount from `users` where app = '".$_SESSION['app']."' and owner = '".$_SESSION['username']."'");
	$records = mysqli_fetch_assoc($sel);
	$totalRecords = $records['allcount'];
	
	## Total number of record with filtering
	$sel = mysqli_query($link,"select count(1) as allcount from `users` WHERE 1 ".$searchQuery." and app = '".$_SESSION['app']."' and owner = '".$_SESSION['username']."'");
	$records = mysqli_fetch_assoc($sel);
	$totalRecordwithFilter = $records['allcount'];
	
	## Fetch records
	$empQuery = "select * from `users` WHERE 1 ".$searchQuery." and app = '".$_SESSION['app']."' and owner = '".$_SESSION['username']."' order by `".$columnName."` ".$columnSortOrder." limit ".$row.",".$rowperpage;
	// echo $empQuery;
